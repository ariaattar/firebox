...
export class FireboxSDK {
    ...
    async shouldIgnoreAnthropicRuntime() {
        await this.ensureLoaded();
    ...
    async setDefaultImage(name, yamlPath, ensureNow = false) {
        await this.ensureLoaded();
    ...
    async prepareBashCommand(command, options = {}) {
        await this.ensureLoaded();
    ...
    async ensureRuntimeReady() {
        ...
        if (!imageExists) {
            ...
            if (!image.yamlPath) {
                throw new Error(`firebox image "${image.name}" is missing. Provide yamlPath via setDefaultImage(name, yamlPath).`);
            }
    ...
    async writeDefaultImageYaml(yamlPath) {
        try {
    ...
    async ensureSessionSandbox(sessionId, workspaceDir) {
